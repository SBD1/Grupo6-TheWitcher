...
def listar_areas_crows_perch():
	...
	sql = "select descricao from area a where a.id_mapa = 2 or a.id_mapa = 5"
	cur.execute(sql)
	rows = cur.fetchall()
	...
	print(" .: Sntrar no cemitério  :. ")
	print(" .: Menu Geral :. ")

	while True:
		option = input("> ")

		if option.lower() not in ['subir a colina', 'entrar na torre', 'menu geral']: 
...
# TODO: implement logic with trigger to check the capacity
def pegar_itens():
	...
	sql = """select item.id from encontrado_em 
		...
		where encontrado_em.id_area = 5 and encontrado_em.id_instancia_item is not null"""
	...
	deletar_instancia_item = """
		delete from encontrado_em where encontrado_em.id_instancia_item is not null and encontrado_em.id_area = 5
	"""
...
def ir_para_torre_lobisomem():
	...
	if not lobisomem: ir_para_torre()
